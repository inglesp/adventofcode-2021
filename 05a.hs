...
solve inp = length $ map fst $ filter (\(_, n) -> n > 1) $ frequencies allCoords
        where entries = map parseLine (lines inp)
              validEntries = filter validEntry entries
              allCoords = concat $ map coords validEntries

parseLine :: String -> ((Int, Int), (Int, Int))
parseLine line = ((read x1, read y1), (read x2, read y2))
        where [[_, x1, y1, x2, y2]] = line =~ "([0-9]+),([0-9]+) -> ([0-9]+),([0-9]+)" :: [[String]]

validEntry :: ((Int, Int), (Int, Int)) -> Bool
validEntry ((x1, y1), (x2, y2)) | x1 == x2 = True
                                | y1 == y2 = True
                                | otherwise = False

coords :: ((Int, Int), (Int, Int)) -> [(Int, Int)]
coords ((x1, y1), (x2, y2)) | (x1 == x2) && (y1 < y2) = zip (repeat x1) [y1..y2]
                            | (x1 == x2) && (y1 > y2) = zip (repeat x1) [y2..y1]
                            | (y1 == y2) && (x1 < x2) = zip [x1..x2] (repeat y1)
                            | (y1 == y2) && (x1 > x2) = zip [x2..x1] (repeat y1)
